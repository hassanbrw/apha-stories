...
function loadVideoSpec(file) {
  ...
  // ASLI BUG (2026-08-18): pehle wala regex `/^##\s*Topic\s*\n([\s\S]*?)(?=\n##|\n*$)/mi`
  // `m` flag ke sath `$` HAR line ke aakhir mein match karta hai, na ke sirf
  // string ke aakhir mein — is liye lazy `[\s\S]*?` topic paragraph ke sirf
  // PEHLE line ke baad hi ruk jata tha (chahe topic 20 lines ka ho). Ye HAR
  // video ke topic ko chupke se sirf pehli line tak kaat raha tha, project
  // ki shuruaat se — sirf ab pakड़ mein aaya jab ek topic ka pehla jumla
  // (bina twist ke) khud kaafi nahi tha model ke liye. Fix: line-by-line
  // scan, "## Topic" ke baad se agli "## " heading (ya file ka end) tak
  // sab kuch uthao — koi multiline-`$` ka masla nahi.
  const lines = raw.split('\n');
  const topicIdx = lines.findIndex(l => /^##\s*Topic\s*$/i.test(l.trim()));
  if (topicIdx !== -1) {
    const body = [];
