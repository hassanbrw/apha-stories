...
function deliverVideo(id, wd) {
  ...
  if (fs.existsSync(thumbDir)) {
    // saare thumbnail variants (thumb_1/2/3.jpg) — sirf pehla nahi, user
    // khud dekh kar behtareen wala chunna chahta hai (2026-08-19)
    const all = fs.readdirSync(thumbDir).filter(f => /\.(jpe?g|png|webp)$/i.test(f)).sort();
    all.forEach((f, i) => items.push({ src: path.join(thumbDir, f), name: `thumbnail_${i + 1}${path.extname(f)}` }));
  }
